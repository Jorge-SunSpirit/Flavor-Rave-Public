		...
		if (controls.ACCEPT && grpControls.members[curSelected].text != "No Replays...")
		{
			...
			if (PlayState.rep.replay.replayGameVer == Replay.version)
			{
				...
				if (PlayState.rep.replay.sm)
					...
					{
						FlxG.stage.application.window.alert("The SM file in this replay does not exist!", "SM Replays");
						return;
					}
				#end
				...
				if (PlayState.isSM)
				{
					...
					{
						FlxG.stage.application.window.alert("Make sure that the SM file is called "
							+ PlayState.pathToSm
							+ "/"
				...
				{
					FlxG.stage.application.window.alert("Failed to load the song! Does the JSON exist?", "Replays");
					return;
				}
