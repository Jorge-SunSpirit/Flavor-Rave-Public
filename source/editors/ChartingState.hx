...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		'Hurt Note',
		'GF Sing',
		'Extra Character Sing',
		'Duo Character Sing',
		'GF Sing Alt',
		'Extra Character Sing Alt',
		'Duo Character Sing Alt',
		'No Animation',
		'Trail Note',
		'Trail Note GF',
		'Trail Note Extra'
	...
	[
		...
		['Force Idle On Beat', "Forces the idle animation to play on beat regardless\non if the animation is still playing.\n\nValue 1: Off <= 0, On >= 1"],
		['Hide Character', "Hides the character.\nValue 1: Character to change (Dad, BF, GF)\nValue 2: Off <= 0, On >= 1"],
		['Hey!', "Plays the \"Hey!\" animation from Bopeebo,\nValue 1: BF = Only Boyfriend, GF = Only Girlfriend,\nSomething else = Both.\nValue 2: Custom animation duration,\nleave it blank for 0.6s"],
		['Set GF Speed', "Sets GF head bopping speed,\nValue 1: 1 = Normal speed,\n2 = 1/2 speed, 4 = 1/4 speed etc.\nUsed on Fresh during the beatbox parts.\n\nWarning: Value must be integer!"],
		['Add Camera Zoom', "Used on MILF on that one \"hard\" part\nValue 1: Camera zoom add (Default: 0.015)\nValue 2: UI zoom add (Default: 0.03)\nLeave the values blank if you want to use Default."],
		['Change Camera Bump frequency', "Changes how often the camera bumps per section \nValue 1: How often (between 1 and 16 default is 16)\nValue 2:Strength of Bump (Default is 1)"],
		...
		['Change Note Skin', "Value 1: Character note skin to change (Dad, BF)\nValue 2: Character's new note skin"],
		['Change Strum Skin', "Value 1: Character note skin to change (Dad, BF)\nValue 2: Character's new strum skin"],
		['Change Ratings', "Value 1: Character rating to change (Dad, BF)\nValue 2: Character's new ratings"],
		['Change Note Splash', "Value 1: Character note splash to change (Dad, BF)\nValue 2: Array data for character's new note splash"],
		['Change Scroll Speed', "Value 1: Scroll Speed Multiplier (1 is default)\nValue 2: Time it takes to change fully in seconds."],
		['Set Property', "Value 1: Variable name\nValue 2: New value"],
		[
			'Move Boyfriend Tween',
			"Move Character position
			Value 1: position - EXAMPLE:0,0
			Value 2: Speed of the tween
			Type 'default' on either of the positions for the default position"
		],
		[
			'Move Opponent Tween',
			"Move Character position
		...
		[
			...
			"Moves the camera by a specified amount whenever a note is hit.\nValue 1: Amount\n\n(BAD ENDING used 15)"
		],
		[
			'Play SFX',
			"Value 1: file name\nValue 2: volume"
		...
		[
			'Call on Luas',
			'Calls on Lua Functions. \nValue 1: Function name\nValue 2: Variables (Separate with ,)'
		],
		[
			'Note',
			'Blank event to attach a note to.\nSet value 2 to true if you want it to appear on screen.'
		],
		[
			'Force BF/DAD on character specific notes',
			'Toggle Extra Character or GF notes to not change who sings.\nGood for Nikku who wants to use another note skin\nDoes not affect speaker chart\nValue 1: True/False - False is default.'
		],
		[
			'Clear Particles',
			'Clears all particles in the particle group'
		],
		[
			'Toggle Character Trail',
			'Enables or Disables a character trail.\nValue 1: Which Character to apply it (Leave blank for all)\nValue 2: True or False, self explanatory'
		],
		[
			'Set Camera Speed',
	...
	**/
	...
	public var vocalsOpponent(get, never):FlxSound;
	...
	{
		return vocalTracks.get(separateVocals ? "player" : "");
	}

	// Either give opponent track or "" track
	function get_vocalsOpponent():FlxSound
	{
		return vocalTracks.get(separateVocals ? "opponent" : "");
	}
	...
	override function create()
	{
		...
		else
		{
			...
			_song = {
				id: null, // set automatically by the game
				song: 'Test',
				notes: [],
				events: [],
				bpm: 150.0,
		...
		for (i in 0...tipTextArray.length) {
			...
			tipText.y += i * 12;
			tipText.setFormat(Language.font.get('vcr'), 14, FlxColor.WHITE, LEFT/*, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK*/);
	...
	function addSongUI():Void
	{
		...
		var reloadSongJson:FlxButton = new FlxButton(reloadSong.x, saveButton.y + 25, "Reload JSON", function()
		{
			openSubState(new Prompt('This action will clear current progress.\n\nProceed?', 0, function(){loadJson(_song.song.toLowerCase()); }, null,ignoreWarnings));
		});

		var loadAutosaveBtn:FlxButton = new FlxButton(reloadSongJson.x, reloadSongJson.y + 25, 'Load Autosave', function()
		{
			PlayState.SONG = Song.parseJSONshit(FlxG.save.data.autosave);
			MusicBeatState.resetState();
		});

		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 25, 'Load Events', function()
		{
		...
		var loadSpeakerJson:FlxButton = new FlxButton(loadEventJson.x, loadEventJson.y + 25, "Load Speaker Chart", function()
		{
			openSubState(new Prompt('This action will clear current progress.\n\nProceed?', 0, function(){loadJson(_song.song.toLowerCase(), true); }, null,ignoreWarnings));
		});
		...
		var extraVersionDropDown = new FlxUIDropDownMenuCustom(stageDropDown.x, stageDropDown.y + 40, FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(character:String)
		{
			_song.player4 = characters[Std.parseInt(character)];
			updateHeads();
		});
		extraVersionDropDown.selectedLabel = _song.player4;
		...
		tab_group_song.add(loadSpeakerJson);
	...
	var check_extraCharSection:FlxUICheckBox;
	...
	function addSectionUI():Void
	{
		...
		check_extraCharSection = new FlxUICheckBox(check_gfSection.x, check_gfSection.y + 22, null, null, "Extra Char section", 100);
		check_extraCharSection.name = 'check_extraChar';
		check_extraCharSection.checked = _song.notes[curSec].extraCharSection;
		...
		tab_group_section.add(check_extraCharSection);
	...
	function addChartingUI() {
		...
		waveformUseOppVoices = new FlxUICheckBox(waveformUseVoices.x + 100, waveformUseInstrumental.y, null, null, "Waveform\n(Opp. Vocals)", 100);
		...
		var check_mute_vocals_opponent = new FlxUICheckBox(check_mute_vocals.x + 120, check_mute_vocals.y, null, null, "Mute Opp. Vocals (in editor)", 100);
		check_mute_vocals_opponent.checked = false;
		check_mute_vocals_opponent.callback = function()
		{
			if(!separateVocals) return;
		...
		voicesOppVolume.value = vocalsOpponent.volume;
	...
	function loadSong():Void
	{
		...
		try
		{
			vocalTracks.set("player", new FlxSound().loadEmbedded(Paths.voices(_song.song, _song.player1.split('-')[0])));
			vocalTracks.set("opponent", new FlxSound().loadEmbedded(Paths.voices(_song.song, _song.player2.split('-')[0])));
		}
		catch (e)
		{
			trace(e + ", resorting to Voices.ogg");
			vocalTracks.clear();
			vocalTracks.set("", new FlxSound().loadEmbedded(Paths.voices(_song.song)));
		}
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				...
				case 'Extra Char section':
					_song.notes[curSec].extraCharSection = check.checked;
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			{
				if(separateVocals) vocalsOpponent.volume = nums.value;
			}
		}
	...
	function updateSectionUI():Void
	{
		...
		check_extraCharSection.checked = sec.extraCharSection;
	...
	function updateHeads():Void
	{
		...
		if (_song.notes[curSec].mustHitSection)
		{
			...
			if (_song.notes[curSec].extraCharSection) leftIcon.changeIcon(_song.player4);
		}
		else
		{
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat(Language.font.get('vcr'), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			if(endThing > i[0] && i[0] >= startThing)
			{
				...
				daText.setFormat(Language.font.get('vcr'), 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
	...
	function loadJson(song:String, ?speaker:Bool = false):Void
	{
		//shitty null fix, i fucking hate it when this happens
		//make it look sexier if possible
		try
		{
		var jsonName:String = song.toLowerCase();
		if (speaker)
			jsonName = 'speaker';

		if (CoolUtil.difficulties[PlayState.storyDifficulty] != CoolUtil.defaultDifficulty && CoolUtil.difficulties[PlayState.storyDifficulty] != null && jsonName == song.toLowerCase()) 
			PlayState.SONG = Song.loadFromJson(jsonName + "-" + CoolUtil.difficulties[PlayState.storyDifficulty], song.toLowerCase());
		else 
			PlayState.SONG = Song.loadFromJson(jsonName, song.toLowerCase());
		...
		{
			openSubState(new Prompt('This Json failed to load.\nSorry :(((((', 0, null, null,ignoreWarnings));
		}
	}
	...
	private function saveLevel()
	{
		...
		var data:String = FlxG.keys.pressed.CONTROL ? Json.stringify(json) : Json.stringify(json, "\t");
