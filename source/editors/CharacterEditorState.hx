...
{
	var char:EditCharacter;
	var ghostChar:EditCharacter;
	var textAnim:FlxText;
	var bgLayer:FlxTypedGroup<FlxSprite>;
	var charLayer:FlxTypedGroup<EditCharacter>;
	...
	override function create()
	{
		...
		add(bgLayer);
		charLayer = new FlxTypedGroup<EditCharacter>();
		add(charLayer);
		...
		textAnim.setFormat(Language.font.get('vcr'), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function addSettingsUI() {
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			...
			var characters:Array<EditCharacter> = [char, ghostChar];
	...
	function loadChar(isDad:Bool, blahBlahBlah:Bool = true) {
		...
		while(i >= 0) {
			var memb:EditCharacter = charLayer.members[i];
			if(memb != null) {
				memb.kill();
		...
		charLayer.clear();
		ghostChar = new EditCharacter(0, 0, daAnim, !isDad);
		ghostChar.debugMode = true;
		ghostChar.alpha = 0.6;

		char = new EditCharacter(0, 0, daAnim, !isDad);
