...
class DiffCalc
{
	...
	public static function CalculateDiff(song:SwagSong, ?accuracy:Float = .93, ?opponentmode:Bool = false)
	{
		// cleaned notes
		...
		for (i in song.notes) // sections
		{
			...
			{
				var gottaHitNote:Bool = opponentmode ? !i.mustHitSection : i.mustHitSection;

				if (ii[1] >= 3 && gottaHitNote)
					cleanedNotes.push(new SmallNote(ii[0] / ClientPrefs.getGameplaySetting('songspeed', 1), Math.floor(Math.abs(ii[1]))));
		...
		return CoolUtil.floorDecimal(chisel(accuracy, hand_diffOne, hand_diffTwo, point_npsOne, point_npsTwo, maxPoints), 2);
	}
